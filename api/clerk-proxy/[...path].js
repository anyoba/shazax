export default async function handler(req, res) {
  ...
  const pathname = incomingPath.replace(/^\/__clerk/, '').replace(/^\/api\/__clerk/, '').replace(/^\/api\/clerk-proxy/, '') || '/';
  const targetUrl = new URL(`${pathname}${requestUrl.search}`, 'https://frontend-api.clerk.dev');

  console.log('[clerk-proxy] req.url =', req.url);
  console.log('[clerk-proxy] incomingPath =', incomingPath);
  console.log('[clerk-proxy] pathname =', pathname);
  console.log('[clerk-proxy] targetUrl =', targetUrl.toString());
